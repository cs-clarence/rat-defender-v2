...
> = {
    ...
    initialValue: Signal<Maybe<FieldPathValue<TFieldValues, TFieldName, true>>>;
    startValue: Signal<Maybe<FieldPathValue<TFieldValues, TFieldName, true>>>;
    value: Signal<Maybe<FieldPathValue<TFieldValues, TFieldName, true>>>;
    error: Signal<string>;
    ...
    // Other
    validate: ValidateField<FieldPathValue<TFieldValues, TFieldName, true>>[];
    transform: TransformField<FieldPathValue<TFieldValues, TFieldName, true>>[];
...
export type RawFieldState<
    ...
    TFieldName extends FieldPath<TFieldValues>,
> = {
    startValue: Maybe<FieldPathValue<TFieldValues, TFieldName, true>>;
    value: Maybe<FieldPathValue<TFieldValues, TFieldName, true>>;
    error: string;
    touched: boolean;
    dirty: boolean;
};

export type IntersectValuesBase<T extends FieldValues> = (
    T extends unknown
        ? (x: T) => void
        : T
...
export type IntersectValues<T extends FieldValues> = (
    T extends FieldValues
        ? {
              [K in keyof T]: T[K] extends FieldValues
                  ? IntersectValues<T[K]>
                  : T[K];
          }
        : T
) extends infer O extends FieldValues
    ? IntersectValuesBase<O>
    : T;
