            ...
            if (
                ...
                batch(() => {
                    ...
                    fieldArrayNames.forEach((fieldArrayName) => {
                        ...
                        // biome-ignore lint/style/noNonNullAssertion: <explanation>
                        const fieldArrayState = getFieldArrayState(
                            form,
                            fieldArrayName,
                        )!;

                        // Get index of current field array
                        const fieldArrayIndex = getPathIndex(
                            name,
                            fieldArrayName,
                        );
                    ...
                    fieldArrayStateMap.forEach(
                        (fieldArrayState, fieldArrayName) => {
                            setFieldArrayState(
                                form,
                    ...
                    fieldArray.items.set((prevItems) => {
                        ...
                        nextItems.splice(
                            toIndex,
                            0,
                            nextItems.splice(fromIndex, 1)[0] ?? 0,
                        );
                        return nextItems;
                    });
