...
mod mat_ext {
    ...
    pub trait MatExt {
        ...
        #[cfg(test)]
        fn new_randn_nd<T>(shape: &[usize]) -> Result<Self, Self::Error>
        where
            Self: Sized,
            T: OpenCvElement;
    }
    ...
    impl MatExt for core_cv::Mat {
        ...
        fn as_slice<T>(&self) -> eyre::Result<&[T]>
        where
            T: OpenCvElement,
        {
            eyre::ensure!(self.depth() == T::DEPTH, "element type mismatch");
            eyre::ensure!(self.is_continuous(), "Mat data must be continuous");
        ...
        #[cfg(test)]
        fn new_randn_2d(rows: i32, cols: i32, typ: i32) -> eyre::Result<Self>
        where
            Self: Sized,
        {
        ...
        #[cfg(test)]
        fn new_randn_nd<T>(shape: &[usize]) -> eyre::Result<Self>
        where
            T: OpenCvElement,
        {
    ...
    T: core_cv::DataType,
{
    ...
    fn try_from_cv(from: &core_cv::Mat) -> eyre::Result<Self> {
        let slice = from.data_typed::<T>()?;
        eyre::ensure!(slice.len() == 2, "invalid length");
        let point = Self {
            x: slice[0],
            y: slice[1],
        };
    ...
    T: core_cv::DataType,
{
    ...
    fn try_from_cv(from: &core_cv::Mat) -> eyre::Result<Self> {
        let slice = from.data_typed::<T>()?;
        eyre::ensure!(slice.len() == 3, "invalid length");
        let point = Self {
            x: slice[0],
            y: slice[1],
            z: slice[2],
    ...
    T: core_cv::DataType,
{
    ...
    fn try_from_cv(from: &core_cv::Point_<T>) -> eyre::Result<Self> {
        let core_cv::Point_ { x, y, .. } = *from;
        let mat = core_cv::Mat::from_slice(&[x, y])?.clone_pointee();
        Ok(mat)
    }
}
    ...
    T: core_cv::DataType,
{
    ...
    fn try_from_cv(from: &core_cv::Point3_<T>) -> eyre::Result<Self> {
        let core_cv::Point3_ { x, y, z, .. } = *from;
        let mat = core_cv::Mat::from_slice(&[x, y, z])?.clone_pointee();
        Ok(mat)
    }
}
